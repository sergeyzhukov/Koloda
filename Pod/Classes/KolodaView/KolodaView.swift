...
public class KolodaView: UIView, DraggableCardDelegate {
  ...
  private func swipedAction(direction: SwipeResultDirection) {
    ...
    if currentCardNumber  >= countOfCards {
      currentCardNumber = 0
    }
    currentCardNumber++
    var indexOfNextLastCard: Int
    if currentCardNumber +  countOfVisibleCards - 1 >= countOfCards {
      indexOfNextLastCard = currentCardNumber - (countOfCards - countOfVisibleCards)  - 1
    } else {
      indexOfNextLastCard = currentCardNumber +  countOfVisibleCards - 1
    }

    if let dataSource = dataSource {

      let lastCardContentView = dataSource.koloda(self, viewForCardAtIndex: UInt(indexOfNextLastCard))
      let lastCardOverlayView = dataSource.koloda(self, viewForCardOverlayAtIndex: UInt(indexOfNextLastCard))
      var lastCardFrame = frameForCardAtIndex(UInt(indexOfNextLastCard))
